...
/**
 * <p>
 * This class is thread-safe, but provides no guarantee on order of operations. For example, it is possible to
 * {@link #notify(Object)} and then {@link #resolve(Object)} such that the <code>Deferred</code> and its
...
public class Deferred<T> implements Promise<T>, Runnable {
    /**
     * A <code>Pipe</code> is a special type of <code>Deferred</code> which listens for events from another
     * <code>Deferred</code> and filters the values, changes the events, or both before applying them to itself.
     * 
     * @author Bryan Harclerode
     * @created Nov 30, 2012
     * @param <U>
     *            The type of object returned by this <code>Pipe</code>'s resolve filter.
     */
    private class Pipe<U> extends Deferred<U> implements Delegate<Void, Promise<T>> {
        /**
         * The delegate invoked to filter the resolve value when the parent promise resolves
         */
        private final Delegate<Promise<U>, T>         resolveFilter;
        /**
         * The delegate invoked to filter the error value when the parent promise rejects
         */
        private final Delegate<Promise<U>, Throwable> rejectFilter;
        /**
         * The delegate invoked to filter each progress value when the parent promise publishes a progress update.
         */
        private final Delegate<Object, Object>        progressFilter;
        /**
         * A delegate which can be attached to another promise via {@link Promise#onDone(Delegate)} and pass the values
         * on to this <code>Pipe</code>. This is used to hook the <code>Pipe</code> up to any promises returned by the
         * filters.
         */
        ...
        public <P extends Promise<T>> Void invoke(P promise) {
            ...
            if ( !promise.isDone() && progressFilter != null) {
            ...
            // Only reaches here if the promise is done, which means the promise is resolved or rejected
            // If either filter throws an exception, reject the Pipe with that exception
            // Otherwise, try to obtain a proxy (replacement) promise from the filter.
            // If there is no proxy promise, resolve or reject the promise as appropriate;
            // else, pipe the proxy promise's events to the pipe's events unfiltered.
            try {
                Promise<U> proxy = null;
                if (promise.isRejected()) {
                    proxy = rejectFilter.invoke(promise.getError());
                    ...
                    proxy = resolveFilter.invoke(promise.getValue());
                ...
                if (proxy != null) {
                    proxy.onDone(proxyPipe);
                }
            } catch (Throwable e) {
                // If any of the handlers throw an exception, reject the pipe with it.
                reject(e);
            }
            return null;
        }
    ...
    /**
     * Returns a {@link Promise} which resolves when all of the arguments to this function resolve, or rejects if any of
     * the arguments to this method reject.
     * 
     * @param promises
     *            The <code>Promise</code>s or objects to wait on.
     * @return A new {@link Promise} which resolves when all of the parameter promises are resolved.
     */
        ...
        Delegate<Void, Promise<Object>> handler = new Delegate<Void, Promise<Object>>() {
            ...
            public <P extends Promise<Object>> Void invoke(P params) {
                ...
                if (params.isRejected()) {
                    ...
                    // wait until every promise has resolved, and then resolve this promise
    ...
    private T                                                 value;
    /**
     * The value that this <code>Deferred</code> was rejected with, if the promise was rejected. If the promise was
     * resolved or has not completed yet, then this value is undefined.
     */
    private Throwable                                         error;
    /**
     * The last progress value that was set for this <code>Deferred</code> using {@link #notify()}.
     */
    private Object                                            progress          = null;

    /**
     * Creates a new <code>Deferred</code> in the {@link State#DEFERRED DEFERRED} state.
     */
    public Deferred() {
    }

    /**
     * Creates a new <code>Deferred</code> and immediately resolves it with <code>object</code>.
     * 
     * @param object
    ...
    /**
     * Updates the current progress value for this <code>Deferred</code> and then triggers a progress notification to
     * all progress pipes and callbacks currently registered with this <code>Deferred</code>.
     * 
     * @param progress
    ...
    public Promise<T> notify(Object progress) {
        ...
        for (Delegate<Void, Promise<T>> progressCallback : progressCallbacks) {
    ...
    /**
     * Rejects this <code>Deferred</code> with the specified error, and then executes any callbacks registered with this
     * deferred's <code>Promise</code>. Any threads waiting on the promise with {@link #getError()} will also be woken
     * and the rejection error returned, while any threads waiting on {@link #getValue()} will also be woken, but an
     * exception thrown.
     * 
     * @param error
     ...
     *             If this <code>Deferred</code> has already been resolved or rejected.
     * @see #getError()
     * @see #getValue()
     * @threadsafe
     */
    public Promise<T> reject(Throwable error) {
        // Throw an exception if we can't update the promise state (because another thread is)
        if ( !state.compareAndSet(State.DEFERRED, State.REJECTED))
    ...
    /**
     * Resovles this <code>Deferred</code> with the specified value, and then executes any callbacks registered with
     * this <code>Promise</code>. Any threads waiting on the promise with {@link #getValue()} will also be woken and the
     * resolved value returned, while any threads waiting on {@link #getError()} will also be woken, but an exception
     * thrown.
     * 
     * @param object
    ...
    public Promise<T> resolve(T object) {
        // Throw an exception if we can't update the promise state (because another thread is)
        if ( !state.compareAndSet(State.DEFERRED, State.RESOVLED))
            throw new IllegalStateException("Promise already finished");
        this.value = object;
        doCallbacks();
        return this;
    ...
    /**
     * Checks if this <code>Deferred</code> has been resolved, and if so, executes any pending callbacks as appropriate.
     * 
     * @threadsafe
     */
    @Override
    public void run() {
        ...
        // No synchronization needed because state can't go from done --> not done.
        if ( !isDone())
            return;
        ...
        // execute all the rest of the callbacks, emptying the queues in the process.
        Delegate<Void, Promise<T>> alwaysCallback = null;
        while ((alwaysCallback = alwaysCallbacks.poll()) != null) {
            try {
        ...
        while ((resolveCallback = resolveCallbacks.poll()) != null) {
            if (isResolved()) {
                try {
        ...
        while ((rejectCallback = rejectCallbacks.poll()) != null) {
            if (isRejected()) {
                try {
    ...
    public String toString() {
        return "{" + state.get() + (isResolved() ? ("," + getValue()) : isRejected() ? ("," + getError()) : "") + "}";
    }

    /**
     * Suspends the current thread until this <code>Deferred</code>'s {@link Promise} resolves.
     * 
     * @threadsafe
     * @throws RuntimeException
     *             if the thread is interrupted while waiting. The <code>cause</code> contains the actual
